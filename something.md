...
## 1. Docking Test in Simulation

We initially built a simple dashboard to test our docking, tune parameters, and see how docking integrated with the exploration subsystem.

We later switched to Foxglove for simulation, using annotations on a debug topic to visualise and debug each stage of the docking FSM.

## 2. Methods We Tried

We went through several stages. We first switched from QR-based fiducial anchoring to ArUco-based detection because of its simplicity, fast detection speed, and low computational overhead (as stated in ConOps / PDR). In Week 12 we briefly tried Nav2-based docking for better collision avoidance and navigation once the marker had been detected.

Although we attempted Nav2-based docking (`opennav_docking`), by the time the base was responding reliably to velocity commands there was not enough time left to properly configure the dock plugin and tune its controller for our drivetrain. Rather than introduce a half-integrated dependency on the critical path, we committed to a direct ArUco visual-servo approach that we could build, tune, and debug end-to-end ourselves.

## 3. Final Implementation

The docking behaviour lives in `simple_aruco_dock.py` (`SimpleArucoDock`). It does visual servoing directly from camera pixels to `/cmd_vel` — no Nav2 action, no TF lookup, no station frame, only the marker pose in the camera frame and a PI loop on top of it. A ROS 2 service call (`/aruco_dock/dock_to_a` or `/dock_to_b`) triggers the approach; completion is reported on `/aruco_dock/done`.

### 3.1 Camera Implementation

The camera is a generic USB webcam driven by `ros-jazzy-usb-cam`, publishing `sensor_msgs/Image` on `/usb_cam/image_raw` and `sensor_msgs/CameraInfo` on `/usb_cam/camera_info`. We ran an intrinsic calibration with the standard `camera_calibration` checkerboard pipeline; the resulting `K` matrix and distortion vector are cached once in `_info_cb` and reused for `solvePnP`. Without this calibration the marker-distance estimate is biased by several centimetres — well over our 4 cm docking tolerance.
...
2. Runs `aruco.detectMarkers` once, splitting the result into two uses: a passive scan that republishes the station pose on `/station_a_pose` whenever the marker is seen, and an active-approach branch that matches the commanded target ID.
...
4. Derives from `tvec`: bearing = `atan2(tx, tz)`, distance = `tz − camera_forward_offset` (the camera sits 8 cm ahead of `base_link`, so the controller stops the *robot* — not the *camera* — 30 cm from the marker), lateral offset = `tx`, and marker-normal yaw from `cv2.Rodrigues(rvec)`.

### 3.3 Control Algorithm

Inside `APPROACHING`, the control loop is:
...
### 3.6 Post-Dock Sequence

Because the launcher is offset from the robot's front, the approach ends with a two-step alignment:

- **Post-turn** — rotate CCW so the marker normal ends up at −90° in the camera frame (wall on the robot's left). The turn magnitude is computed in the camera frame from `normal_yaw` and executed in odom with a small P controller on yaw.
- **Post-shift** — after turning, the camera no longer sees the marker. The required shift is derived in closed form from the *remembered* pre-turn lateral offset and the turn angle:
...
## 4. Step-by-Step Algorithm and FSM
...
### 4.3 Per-Frame Algorithm in `APPROACHING`
